...
**Done (2026-07-31)**:
- `02-doc/current.md` — new F35 session entry + layering summary; date bumped.
- Literate: `03-nav_manager_node.md` + `05-nav_manager.md` deleted with their
  modules (T06); `09-explorer_manager_node.md` regenerated to v2.0 (ExploreArea
  action server, `/intent` dropped, MultiThreadedExecutor); `00-overview.md`
  updated (intent-nav moved out, reading order fixed).
- `README.md` + `CLAUDE.md` — dome_nav framed as primitives-only; related
  packages (dome_mission / dome_nav_msgs / dome_semantic_msgs) listed; layering
  noted.
- F33/TF33: TF33 T05 already marked "superseded by F35" (consumer moved to
  dome_mission) — verified consistent, no change needed.
- dome_mission's own literate (`mission_fsm` / `intent_parser` / `label_resolver`
  / `mission_node`) is that package's future task, not dome_nav T08 scope.
