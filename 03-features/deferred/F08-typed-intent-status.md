...
> Relocated from dome_nav → dome_mission (2026-07-31, F35 staged move). Post-extraction,
> `/intent` + `nav_status` are dome_mission's contract, so this concern lives here.
> Still cross-package (dome_control/dome_vision produce `/intent`) — needs lockstep coordination.
